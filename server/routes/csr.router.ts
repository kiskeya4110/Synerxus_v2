...
csrRouter.get("/csr/dashboard", authMiddleware, requireCSRAccess, queueMiddleware('heavy'), async (req: Request, res: Response) => {
  try {
    ...
    if (timePeriod) {
      ...
      logger.info(`[CSR Dashboard] Time filter applied: ${timePeriod}, filtering from ${startDate.toISOString()} to ${endDate.toISOString()}`);
...
csrRouter.get("/csr/notifications", authMiddleware, requireCSRAccess, async (req: Request, res: Response) => {
  try {
    ...
    logger.error("Error fetching CSR notifications:", err);
...
csrRouter.get("/csr/impact-reporting/export/csv", authMiddleware, requireCSRAccess, requirePlanFeature("csvExport"), exportRateLimiter, queueMiddleware('heavy'), async (req: Request, res: Response) => {
  try {
    ...
    if (!impactResponse.ok) {
      logger.error(`Impact reporting API returned ${impactResponse.status}`);
    ...
    try {
      ...
      logger.error("Failed to parse impact data JSON:", jsonErr);
