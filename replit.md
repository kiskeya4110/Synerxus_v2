...
- **Impact Over Time Chart**: Multi-line Chart.js visualization displaying Volunteer Hours, People Impacted, and Algorithm-Evaluated Impact Score across 7-month rolling window. Implements `coerceNumber()` helper with `Number.isFinite()` guard to safely convert string/numeric values from API, preventing concatenation bugs and NaN propagation. Custom Chart.js legend configuration renders solid line segments (with dashed line for algorithm score) instead of filled rectangles, using transparent `fillStyle` and `strokeStyle` matching dataset colors for accurate visual representation.
