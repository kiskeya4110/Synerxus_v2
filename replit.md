...
- **Automatic Project Completion Tracking**: Backend automatically recalculates project completion percentage when tasks are created or updated. Uses case-insensitive status matching (`toLowerCase()`) to count completed tasks. Calculation: `(completedTasks / totalTasks) * 100`. Updates persisted to `projects.completionPercentage` field and broadcast via WebSocket for real-time UI updates. Dashboard and all project views display authoritative database value instead of client-side calculations (Nov 2025).
...
- **SDG Mapping**: Visual representation and tracking of project alignment with UN SDGs, with multi-tenant security. Features interactive **spider web (radar) chart** with **logarithmic scale transformation** comparing organization's selected SDG focus areas from Settings (blue baseline) vs actual project distribution (orange-gold data). Chart uses actual project counts (1 per project) with log10(x+1) transformation for visibility when comparing vastly different values. Tick labels display actual project counts via inverse transformation. **Project Statistics Dashboard** (Nov 2025) - displays four summary cards above radar chart showing Total Projects, Completed Projects, Active Projects, and Average Completion percentage aggregated from organization's projects. **Enhanced Project Cards** - Connected Projects section now displays completion percentage with progress bar, location info, and related SDG badges showing cross-SDG alignment. **Interactive Connected Projects** - clicking any SDG goal displays related projects with clickable cards that navigate to project detail pages.
...
- **Impact Visualization**: Interactive impact dashboard with Before/After comparison, clickable aggregated impact metrics, and real-time Chart.js visualizations, including comprehensive empty states. **Impact Stories Project Selector** (Nov 2025) - organization-scoped dropdown allows filtering impact stories by specific project or viewing all projects. Only projects with recorded impact metrics generate stories; selecting projects without data displays clear empty state message.
...
- **Core Management Pages**: Dedicated interfaces for Projects, Tasks, Volunteers, Organizations, Calendar, Opportunities, and Applications. **Projects page now organization-scoped** - fetches only the logged-in organization's projects using `userId` parameter. Full CRUD capabilities with CreateProjectDialog (comprehensive intake form), EditProjectDialog, and DeleteProjectDialog. **Opportunity Management** (Nov 2025) - Organizations can edit, delete, and manage all posted opportunities directly from Projects & Tasks page with Edit/Delete buttons. Project linking feature allows opportunities to be connected to specific projects for better task management. Linked project names displayed prominently on opportunity cards.
...
- **Volunteer Opportunities System**: Full-featured posting and discovery with AI recommendations and application tracking. Includes an enhanced project intake form for comprehensive data collection for AI matching. **Project-Opportunity Linking** (Nov 2025) - Opportunities can be linked to specific projects via optional projectId field. Organizations select project from dropdown during opportunity creation/editing. Linked project names display on opportunity cards in Projects & Tasks page. Volunteers can apply to opportunities through Discover Opportunities page (`/discover-opportunities`) and Dashboard "Find Opportunities" tab using ApplicationDialog component.
