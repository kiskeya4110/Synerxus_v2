...
import { extractSdgsFromProjects, filterValidSdgs, compareSdgArrays } from "@/lib/utils";
import { Users, Clock, CheckSquare, Globe, Building2, Award, TrendingUp, Target, Briefcase, AlertCircle, Zap, FileText, BarChart3, Sparkles, MapPin } from "lucide-react";
import {
  LineChart,
  Line,
  XAxis,
...
// SDG Colors and Names for charts and displays
...
export default function Dashboard() {
  ...
  const [showEnhancedKpiModal, setShowEnhancedKpiModal] = useState<'hours' | 'projects' | 'skills' | 'sdgs' | 'aiu' | null>(null);
  ...
  const sdgMetrics = useMemo(() => {
    ...
    return {
      ...
      alignedSdgs,
      uncommittedWork,
      uncommittedContributions
    };
  }, [dashboardData?.volunteerProfile, filteredData.projects]);
  ...
  const impactOverTimeData = useMemo(() => {
    ...
    const verificationMultiplier = 0.9;
    ...
    if (Array.isArray(serverMonthlyData) && serverMonthlyData.length > 0) {
      return serverMonthlyData.map((item: any) => {
        ...
        return { month: monthLabel, hours, peopleReached: peopleImpacted, aiu };
      });
    }
    ...
    return monthNames.slice(0, currentMonth + 1).map((month, idx) => {
      const monthActivities = (filteredData.activities || []).filter((a: any) => {
      ...
      return { month, hours, peopleReached: 0, aiu: 0 };
    });
  }, [dashboardData?.monthlyImpactData, filteredData.activities]);

  // SDG Distribution data with real metrics (hours per SDG)
  const sdgDistribution = useMemo(() => {
    const safeProjects = Array.isArray(filteredData.projects) ? filteredData.projects : [];
    const safeActivities = Array.isArray(filteredData.activities) ? filteredData.activities : [];
    ...
    safeProjects.forEach((p: any) => {
      if (p?.id && Array.isArray(p?.sdgGoals)) {
        projectSdgMap[p.id] = filterValidSdgs(p.sdgGoals);
      }
    });
    ...
    return Object.entries(sdgHours)
      ...
      .slice(0, 8);
  }, [filteredData.projects, filteredData.activities]);

  // Fact-based AI insights generator
  const factBasedInsights = useMemo(() => {
    const safeProjects = Array.isArray(filteredData.projects) ? filteredData.projects : [];
    const volunteerSkills = dashboardData?.volunteerProfile?.skills || [];
    let summary = "";

    // Hours insight
    const hoursInfo = kpis.hours > 0
      ? `You've contributed ${kpis.hours} volunteer hours`
      : "Start logging hours to track your impact";

    // Projects insight
    const projectInfo = kpis.activeProjects > 0
      ? ` across ${kpis.activeProjects} project${kpis.activeProjects !== 1 ? 's' : ''}`
      : "";

    // SDG insight
    let sdgInsight = "";
    if (sdgMetrics.sdgsContributed > 0) {
      sdgInsight = ` You're contributing to ${sdgMetrics.sdgsContributed} SDG${sdgMetrics.sdgsContributed !== 1 ? 's' : ''}.`;
    }
    if (sdgMetrics.alignedSdgs.length > 0) {
      sdgInsight += ` ${sdgMetrics.alignedSdgs.length} of your committed goals are active.`;
    }
    if (sdgMetrics.uncommittedContributions.length > 0) {
      sdgInsight += ` Bonus: Contributing to ${sdgMetrics.uncommittedContributions.length} additional SDG${sdgMetrics.uncommittedContributions.length !== 1 ? 's' : ''}!`;
    }

    // Skills insight
    const usedSkillsArray: string[] = [];
    safeProjects.forEach((project: any) => {
      ...
      volunteerSkills.forEach((skill: string) => {
        ...
        if (isUsed && !usedSkillsArray.includes(skill)) {
          usedSkillsArray.push(skill);
        }
      });
    });

    let skillsInsight = "";
    if (usedSkillsArray.length > 0) {
      skillsInsight = ` Your ${usedSkillsArray.slice(0, 2).join(' and ')} skill${usedSkillsArray.length > 1 ? 's are' : ' is'} making a difference.`;
    }

    summary = `${hoursInfo}${projectInfo}.${sdgInsight}${skillsInsight}`;

    return {
      summary,
      usedSkills: usedSkillsArray,
      hasPositiveTrend: impactOverTimeData.length >= 2 &&
        impactOverTimeData[impactOverTimeData.length - 1]?.hours > impactOverTimeData[impactOverTimeData.length - 2]?.hours
    };
  }, [kpis, sdgMetrics, filteredData.projects, dashboardData?.volunteerProfile?.skills, impactOverTimeData]);
  ...
  return (
    <PageTransition>
      ...
      {/* SDG Impact Report Section - Enhanced with Interactive Coverage */}
      {dashboardType === "volunteer" && (sdgDistribution.length > 0 || sdgMetrics.sdgsCommitted > 0) && (
        <Card className="mb-6 border-blue-200 dark:border-blue-800 bg-gradient-to-br from-white to-blue-50/30 dark:from-gray-900 dark:to-blue-950/20">
          <CardHeader className="pb-3">
            <div className="flex items-center justify-between">
              <CardTitle className="text-base flex items-center gap-2">
                <div className="p-2 bg-blue-100 dark:bg-blue-900/40 rounded-lg">
                ...
                <div>
                  <span className="block">SDG Impact Report</span>
                  <span className="text-xs font-normal text-gray-500 dark:text-gray-400">Your contribution to UN Sustainable Development Goals</span>
                </div>
              </CardTitle>
              <span className="text-xs bg-blue-50 dark:bg-blue-900/40 text-blue-700 dark:text-blue-300 px-2 py-1 rounded-full font-medium border border-blue-100 dark:border-blue-800">
                UN Aligned
              </span>
            </div>
          </CardHeader>
          <CardContent>
            <div className="space-y-4">
              ...
              <div className="grid grid-cols-4 gap-3">
                <button
                  onClick={() => setShowEnhancedKpiModal('sdgs')}
                  className="text-center p-3 rounded-lg bg-gray-50 dark:bg-gray-800/50 hover:bg-gray-100 dark:hover:bg-gray-800 transition-all active:scale-95"
                >
                  <div className="text-2xl font-bold text-blue-600 dark:text-blue-400">{sdgMetrics.sdgsCommitted}</div>
                  <div className="text-xs text-gray-600 dark:text-gray-400 font-medium">SDG Goals</div>
                  <div className="text-[10px] text-gray-400 dark:text-gray-500">Committed</div>
                </button>
                <button
                  onClick={() => setShowEnhancedKpiModal('hours')}
                  className="text-center p-3 rounded-lg bg-gray-50 dark:bg-gray-800/50 hover:bg-gray-100 dark:hover:bg-gray-800 transition-all active:scale-95"
                >
                  <div className="text-2xl font-bold text-emerald-600 dark:text-emerald-400">{kpis.hours}</div>
                  <div className="text-xs text-gray-600 dark:text-gray-400 font-medium">Hours</div>
                  <div className="text-[10px] text-gray-400 dark:text-gray-500">Volunteered</div>
                </button>
                <button
                  onClick={() => setShowEnhancedKpiModal('projects')}
                  className="text-center p-3 rounded-lg bg-gray-50 dark:bg-gray-800/50 hover:bg-gray-100 dark:hover:bg-gray-800 transition-all active:scale-95"
                >
                  <div className="text-2xl font-bold text-purple-600 dark:text-purple-400">{kpis.activeProjects}</div>
                  <div className="text-xs text-gray-600 dark:text-gray-400 font-medium">Projects</div>
                  <div className="text-[10px] text-gray-400 dark:text-gray-500">Contributing</div>
                </button>
                <button
                  onClick={() => setShowEnhancedKpiModal('aiu')}
                  className="text-center p-3 rounded-lg bg-gray-50 dark:bg-gray-800/50 hover:bg-gray-100 dark:hover:bg-gray-800 transition-all active:scale-95"
                >
                  <div className="text-2xl font-bold text-amber-600 dark:text-amber-400">
                    {typeof kpis.aiuEarned === 'number' ? kpis.aiuEarned.toFixed(2) : kpis.aiuEarned || '0.00'}
                  </div>
                  <div className="text-xs text-gray-600 dark:text-gray-400 font-medium">AIUs</div>
                  <div className="text-[10px] text-gray-400 dark:text-gray-500">Impact Units</div>
                </button>
              </div>
              ...
              <div className="bg-white/70 dark:bg-gray-800/50 rounded-lg p-3">
                <div className="flex items-center justify-between mb-2">
                  <span className="text-xs text-gray-600 dark:text-gray-400 font-medium">SDG Coverage</span>
                  <span className="text-xs text-blue-600 dark:text-blue-400 font-bold">
                    {sdgMetrics.sdgsCommitted} of 17 Goals ({Math.round((sdgMetrics.sdgsCommitted / 17) * 100)}%)
                  </span>
                </div>
                <div className="flex gap-[2px]">
                  {Array.from({ length: 17 }, (_, i) => i + 1).map((sdgNum) => {
                    const isCommitted = sdgMetrics.committedSdgs.includes(sdgNum);
                    const sdgData = sdgDistribution.find((s: any) => s.sdg === sdgNum);
                    const hasContributed = !!sdgData && sdgData.value > 0;
                    return (
                      <button
                        key={sdgNum}
                        onClick={() => (isCommitted || hasContributed) && setShowSdgModal(sdgNum)}
                        className={`flex-1 h-4 rounded-sm transition-all ${(isCommitted || hasContributed) ? 'hover:scale-y-150 cursor-pointer' : 'cursor-default'}`}
                        style={{
                ...
                <div className="flex items-center gap-4 mt-2 text-[10px] text-gray-500 dark:text-gray-400">
                  ...
                  <div className="flex items-center gap-1">
                    <div className="w-3 h-3 rounded-sm bg-blue-500/50 border border-blue-500"></div>
              ...
              {sdgDistribution.length > 0 && (
                <div className="bg-white dark:bg-gray-800 rounded-xl p-4 border border-blue-100 dark:border-blue-800">
                  <div className="flex justify-between items-center mb-3">
                    <span className="text-sm text-gray-700 dark:text-gray-300 font-medium">Hours Distribution by SDG</span>
                    <span className="text-emerald-700 dark:text-emerald-400 text-[10px] px-2 py-0.5 bg-emerald-50 dark:bg-emerald-900/30 rounded-full font-medium">
                      Live Data
                    </span>
                  </div>
                  <div className="h-48">
                        ...
                        <Tooltip
                          content={({ active, payload }) => {
                            if (active && payload && payload.length) {
                              ...
                              return (
                                <div className="bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-xl shadow-xl p-3 text-xs min-w-[180px]">
                                  <div className="flex items-center gap-2 mb-2 pb-2 border-b border-gray-100 dark:border-gray-700">
                                    ...
                                    <div>
                                      <div className="font-bold text-gray-800 dark:text-white">SDG {data.sdg}</div>
                                      <div className="text-gray-500 dark:text-gray-400 text-[10px]">{data.name}</div>
                                    </div>
                                  </div>
                                  <div className="space-y-1">
                                    <div className="flex justify-between"><span className="text-gray-500 dark:text-gray-400">Hours:</span><span className="font-bold text-blue-600 dark:text-blue-400">{data.value}</span></div>
                                    <div className="flex justify-between"><span className="text-gray-500 dark:text-gray-400">Share:</span><span className="font-bold text-emerald-600 dark:text-emerald-400">{percentage}%</span></div>
                                    <div className="flex justify-between"><span className="text-gray-500 dark:text-gray-400">Projects:</span><span className="font-bold text-purple-600 dark:text-purple-400">{data.projectCount}</span></div>
                                  </div>
                                </div>
                              );
                            }
                  ...
                  <div className="flex flex-wrap gap-3 mt-3 pt-3 border-t border-gray-100 dark:border-gray-700">
                    {sdgDistribution.slice(0, 4).map((sdg) => (
                      ...
                      >
                        ...
                        <span className="text-gray-600 dark:text-gray-400">SDG {sdg.sdg}</span>
                      </button>
                    ))}
                  </div>
                </div>
      ...
      {/* AI Insights Card - Fact-based */}
      {dashboardType === "volunteer" && (
        <Card className="mb-6 border-purple-200 dark:border-purple-800 bg-gradient-to-r from-purple-600 to-pink-600 text-white">
          <CardContent className="p-4">
            ...
            {factBasedInsights.usedSkills.length > 0 && (
              <div className="flex flex-wrap gap-2 mt-3">
                {factBasedInsights.usedSkills.slice(0, 3).map((skill, idx) => (
            ...
            {factBasedInsights.hasPositiveTrend && (
              <div className="flex items-center gap-1 mt-3 text-xs text-white/80">
                <TrendingUp className="w-3 h-3" />
                <span>Your activity is trending upward!</span>
              </div>
            )}
          </CardContent>
        </Card>
      ...
      {dashboardType === "volunteer" && impactOverTimeData.some(d => d.hours > 0) && (
        <Card className="mb-6">
          <CardHeader className="pb-3">
            <CardTitle className="text-base flex items-center gap-2">
          ...
          <CardContent>
            <div className="flex flex-wrap gap-3 mb-3 text-xs">
              ...
              <div className="flex items-center gap-1">
                ...
                <span className="text-gray-600 dark:text-gray-400">People Reached</span>
              </div>
              <div className="flex items-center gap-1">
                <div className="w-3 h-0.5 bg-[#f59e0b]"></div>
                <span className="text-gray-600 dark:text-gray-400">AIUs</span>
              </div>
            </div>
            <div className="h-48">
              <ResponsiveContainer width="100%" height="100%">
      ...
      {/* Enhanced KPI Modal with Charts */}
      {showEnhancedKpiModal && (
        <div className="fixed inset-0 bg-black/50 flex items-center justify-center z-[60] px-4" onClick={() => setShowEnhancedKpiModal(null)}>
          <div className="bg-white dark:bg-gray-900 rounded-xl max-w-md w-full max-h-[80vh] overflow-y-auto shadow-xl" onClick={(e) => e.stopPropagation()}>
            <div className="sticky top-0 bg-white dark:bg-gray-900 border-b border-gray-200 dark:border-gray-700 p-4 flex items-center justify-between rounded-t-xl">
              <h2 className="text-lg font-semibold text-gray-800 dark:text-white">
                {showEnhancedKpiModal === 'hours' && 'Total Hours Logged'}
                {showEnhancedKpiModal === 'projects' && 'Project Overview'}
                {showEnhancedKpiModal === 'skills' && 'Skills Applied'}
                {showEnhancedKpiModal === 'sdgs' && 'SDG Contributions'}
                {showEnhancedKpiModal === 'aiu' && 'Adjusted Impact Units'}
              </h2>
              <button
                onClick={() => setShowEnhancedKpiModal(null)}
                className="text-gray-400 hover:text-gray-600 dark:hover:text-gray-200 text-xl font-medium"
              >
                ✕
              </button>
            </div>
            <div className="p-4 space-y-4">
              {showEnhancedKpiModal === 'hours' && (
                <>
                  <div className="text-center py-4">
                    <div className="text-5xl font-bold text-blue-600 dark:text-blue-400 mb-2">{kpis.hours}</div>
                    <div className="text-gray-500 dark:text-gray-400">Hours Volunteered</div>
                  </div>
                  {impactOverTimeData.some(d => d.hours > 0) && (
                    <div className="bg-gray-50 dark:bg-gray-800 rounded-lg p-3 border border-gray-100 dark:border-gray-700">
                      <div className="text-xs text-gray-500 dark:text-gray-400 mb-2">Monthly Hours Trend</div>
                      <div className="h-20">
                        <ResponsiveContainer width="100%" height="100%">
                          <AreaChart data={impactOverTimeData}>
                            <defs>
                              <linearGradient id="hoursGradientModal" x1="0" y1="0" x2="0" y2="1">
                            ...
                            <YAxis hide />
                            <Area type="monotone" dataKey="hours" stroke="#3b82f6" fill="url(#hoursGradientModal)" strokeWidth={2} />
                          </AreaChart>
                        </ResponsiveContainer>
                      </div>
                    </div>
                  ...
                  <div className="bg-blue-50 dark:bg-blue-900/30 rounded-lg p-4 border border-blue-100 dark:border-blue-800">
                    <div className="text-sm text-gray-700 dark:text-gray-300 space-y-2">
                      <div className="flex justify-between">
                        ...
                        <span className="text-emerald-600 dark:text-emerald-400 font-semibold">{aiuSummary?.totalAiu?.toFixed(2) || kpis.aiuEarned?.toFixed?.(2) || '0.00'}</span>
                      </div>
                      <div className="flex justify-between">
                        <span>Across Projects:</span>
                        <span className="text-gray-800 dark:text-gray-200 font-semibold">{kpis.activeProjects}</span>
                      </div>
                      <div className="flex justify-between">
              ...
              {showEnhancedKpiModal === 'projects' && (
                <>
                  <div className="text-center py-4">
                    <div className="text-5xl font-bold text-green-600 dark:text-green-400 mb-2">{kpis.activeProjects}</div>
                    <div className="text-gray-500 dark:text-gray-400">Total Projects</div>
                  </div>
                  <div className="bg-green-50 dark:bg-green-900/30 rounded-lg p-4 border border-green-100 dark:border-green-800 mb-4">
                    <div className="text-sm text-gray-700 dark:text-gray-300 space-y-2">
                      <div className="flex justify-between">
                        <span>Active:</span>
                        <span className="text-orange-600 dark:text-orange-400 font-semibold">
                          {filteredData.projects.filter((p: any) => p.status?.toLowerCase() === 'active' || p.status?.toLowerCase() === 'in progress').length}
                        </span>
                      </div>
                      <div className="flex justify-between">
                        <span>Completed:</span>
                        <span className="text-green-600 dark:text-green-400 font-semibold">
                          {filteredData.projects.filter((p: any) => p.status?.toLowerCase() === 'completed').length}
                        </span>
                      </div>
                      <div className="flex justify-between">
                        <span>Total Hours:</span>
                        <span className="text-blue-600 dark:text-blue-400 font-semibold">{kpis.hours}</span>
                      </div>
                    </div>
                  </div>
                  <div className="space-y-2">
                    <h3 className="text-gray-800 dark:text-white font-semibold text-sm">Your Projects:</h3>
                    {filteredData.projects.slice(0, 5).map((project: any) => {
                      ...
                      return (
                        <Link key={project.id} href={`/projects/${project.id}`}>
                          <div className="bg-gray-50 dark:bg-gray-800 rounded-lg p-3 border border-gray-200 dark:border-gray-700 hover:shadow-md transition-shadow cursor-pointer">
                            <div className="flex items-center justify-between mb-1">
                              <div className="text-gray-800 dark:text-white font-medium text-sm">{project.name}</div>
                              <span className={`text-xs px-2 py-0.5 rounded-full ${
                                normalizedStatus === 'active' || normalizedStatus === 'in progress' ? 'bg-orange-100 dark:bg-orange-900/40 text-orange-700 dark:text-orange-300' :
                                normalizedStatus === 'completed' ? 'bg-green-100 dark:bg-green-900/40 text-green-700 dark:text-green-300' :
                                'bg-gray-100 dark:bg-gray-700 text-gray-600 dark:text-gray-300'
                              }`}>
                                {displayStatus}
                              </span>
                            </div>
                            <div className="text-xs text-gray-500 dark:text-gray-400 mt-1">{project.organizationName}</div>
              ...
              {showEnhancedKpiModal === 'skills' && (
                <>
                  <div className="text-center py-4">
                    <div className="text-5xl font-bold text-orange-500 dark:text-orange-400 mb-2">{kpis.skills}</div>
                    <div className="text-gray-500 dark:text-gray-400">Skills in Your Arsenal</div>
                  </div>
                  <div className="flex flex-wrap gap-2">
                    {(dashboardData?.volunteerProfile?.skills || []).map((skill: string, idx: number) => (
                      <div key={idx} className="bg-orange-100 dark:bg-orange-900/40 text-orange-700 dark:text-orange-300 px-3 py-1.5 rounded-full text-sm font-medium">
                        {skill}
                      </div>
                    ))}
                    {(!dashboardData?.volunteerProfile?.skills || dashboardData.volunteerProfile.skills.length === 0) && (
                      <p className="text-gray-500 dark:text-gray-400 text-sm">No skills added yet. Update your profile to add skills.</p>
                    )}
                  </div>
                  {factBasedInsights.usedSkills.length > 0 && (
                    <div className="mt-4 p-3 bg-emerald-50 dark:bg-emerald-900/30 rounded-lg border border-emerald-100 dark:border-emerald-800">
                      <div className="text-xs text-emerald-700 dark:text-emerald-300 font-medium mb-2">Skills Being Applied:</div>
                      <div className="flex flex-wrap gap-2">
                        {factBasedInsights.usedSkills.map((skill, idx) => (
                          <span key={idx} className="bg-emerald-100 dark:bg-emerald-900/60 text-emerald-700 dark:text-emerald-300 px-2 py-0.5 rounded-full text-xs">
                            {skill}
                          </span>
                        ))}
                      </div>
              ...
              {showEnhancedKpiModal === 'sdgs' && (
                <>
                  <div className="grid grid-cols-2 gap-4 py-4">
                    <div className="text-center">
                      <div className="text-3xl font-bold text-blue-500 dark:text-blue-400">{sdgMetrics.sdgsCommitted}</div>
                      <div className="text-gray-500 dark:text-gray-400 text-sm">Committed</div>
                    </div>
                    <div className="text-center">
                      <div className="text-3xl font-bold text-emerald-500 dark:text-emerald-400">{sdgMetrics.sdgsContributed}</div>
                      <div className="text-gray-500 dark:text-gray-400 text-sm">Contributed</div>
                    </div>
                  </div>

                  {sdgMetrics.alignedSdgs.length > 0 && (
                    <div className="mb-4">
                      <div className="flex items-center gap-2 mb-2">
                        <div className="w-3 h-3 rounded-full bg-emerald-500"></div>
                        <h3 className="text-gray-800 dark:text-white font-semibold text-sm">Aligned ({sdgMetrics.alignedSdgs.length})</h3>
                        <span className="text-xs text-gray-400 dark:text-gray-500">Committed & Contributing</span>
                      </div>
                      <div className="grid grid-cols-3 gap-2">
                        {sdgMetrics.alignedSdgs.map((sdgNum: number) => {
                          const sdgData = sdgDistribution.find(s => s.sdg === sdgNum);
                          return (
                            <button
                              key={sdgNum}
                              onClick={() => { setShowEnhancedKpiModal(null); setShowSdgModal(sdgNum); }}
                  ...
                  {sdgMetrics.uncommittedWork.length > 0 && (
                    <div className="mb-4">
                      <div className="flex items-center gap-2 mb-2">
                        <div className="w-3 h-3 rounded-full bg-amber-500"></div>
                        <h3 className="text-gray-800 dark:text-white font-semibold text-sm">Not Started ({sdgMetrics.uncommittedWork.length})</h3>
                        <span className="text-xs text-gray-400 dark:text-gray-500">Committed, no work yet</span>
                      </div>
                      <div className="grid grid-cols-3 gap-2">
                        {sdgMetrics.uncommittedWork.map((sdgNum: number) => (
                          <button
                            key={sdgNum}
                            onClick={() => { setShowEnhancedKpiModal(null); setShowSdgModal(sdgNum); }}
                  ...
                  {sdgMetrics.uncommittedContributions.length > 0 && (
                    <div className="mb-4">
                      <div className="flex items-center gap-2 mb-2">
                        <div className="w-3 h-3 rounded-full bg-purple-500"></div>
                        <h3 className="text-gray-800 dark:text-white font-semibold text-sm">Bonus ({sdgMetrics.uncommittedContributions.length})</h3>
                        <span className="text-xs text-gray-400 dark:text-gray-500">Contributing, not committed</span>
                      </div>
                      <div className="grid grid-cols-3 gap-2">
                        {sdgMetrics.uncommittedContributions.map((sdgNum: number) => {
                          const sdgData = sdgDistribution.find(s => s.sdg === sdgNum);
                          return (
                            <button
                              key={sdgNum}
                  ...
                  {sdgMetrics.sdgsCommitted === 0 && sdgMetrics.sdgsContributed === 0 && (
                    <div className="text-center text-gray-400 dark:text-gray-500 py-8">
                      <p>No SDG activity yet.</p>
                      <p className="text-xs mt-2">Update your profile to set SDG goals!</p>
                    </div>
                  )}
                </>
              )}

              {showEnhancedKpiModal === 'aiu' && (
                <>
                  <div className="text-center py-4">
                    <div className="text-5xl font-bold text-amber-500 dark:text-amber-400 mb-2">{aiuSummary?.totalAiu?.toFixed(2) || kpis.aiuEarned?.toFixed?.(2) || '0.00'}</div>
                    <div className="text-gray-500 dark:text-gray-400">Total Adjusted Impact Units</div>
                  </div>
                  <div className="bg-amber-50 dark:bg-amber-900/30 rounded-lg p-4 border border-amber-100 dark:border-amber-800 mb-4">
                    <div className="text-sm text-gray-700 dark:text-gray-300 space-y-2">
                      <div className="flex justify-between">
                        <span>Unique AIU:</span>
                        <span className="text-amber-600 dark:text-amber-400 font-semibold">{aiuSummary?.aiuUnique?.toFixed(2) || '0.00'}</span>
                      </div>
                      <div className="flex justify-between">
                        <span>Session AIU:</span>
                        <span className="text-amber-600 dark:text-amber-400 font-semibold">{aiuSummary?.aiuSessions?.toFixed(2) || '0.00'}</span>
                      </div>
                      <div className="flex justify-between">
                        <span>Verification Rate:</span>
                        <span className="text-emerald-600 dark:text-emerald-400 font-semibold">{Math.min(Math.round(aiuSummary?.verificationRate || 0), 100)}%</span>
                      </div>
                      <div className="flex justify-between">
                        <span>Total Hours:</span>
                        <span className="text-blue-600 dark:text-blue-400 font-semibold">{aiuSummary?.totalHours || kpis.hours}</span>
                      </div>
                    </div>
                  </div>
                  {aiuSummary?.projects && aiuSummary.projects.length > 0 && (
                    <div className="space-y-2">
                      <h3 className="text-gray-800 dark:text-white font-semibold text-sm">AIU by Project:</h3>
                      {aiuSummary.projects.slice(0, 5).map((project: any) => (
                        <Link key={project.projectId} href={`/projects/${project.projectId}`}>
                          <div className="bg-gray-50 dark:bg-gray-800 rounded-lg p-3 border border-gray-200 dark:border-gray-700 hover:shadow-md transition-shadow cursor-pointer">
                            <div className="flex items-center justify-between mb-1">
                              <div className="text-gray-800 dark:text-white font-medium text-sm truncate flex-1">{project.projectName}</div>
                              <span className="text-amber-600 dark:text-amber-400 font-bold text-sm ml-2">{project.aiu?.toFixed(2) || '0.00'} AIU</span>
                            </div>
      ...
      {/* SDG Detail Modal */}
      {showSdgModal && (
        <div className="fixed inset-0 bg-black/50 flex items-center justify-center z-[60] px-4" onClick={() => setShowSdgModal(null)}>
          <div className="bg-white dark:bg-gray-900 rounded-xl max-w-md w-full max-h-[80vh] overflow-y-auto shadow-xl" onClick={(e) => e.stopPropagation()}>
            <div className="sticky top-0 border-b border-gray-200 dark:border-gray-700 p-4 flex items-center justify-between rounded-t-xl" style={{ backgroundColor: SDG_COLORS[showSdgModal] }}>
              <h2 className="text-lg font-semibold text-white">
                SDG {showSdgModal}: {SDG_NAMES[showSdgModal]}
              </h2>
              <button
            ...
            <div className="p-4 space-y-4">
              {(() => {
                const sdgData = sdgDistribution.find(s => s.sdg === showSdgModal);
                const isCommitted = sdgMetrics.committedSdgs.includes(showSdgModal);
                const relatedProjects = filteredData.projects.filter((p: any) =>
                  p.sdgGoals?.includes(showSdgModal)
                );
                return (
                  <>
                    <div className="grid grid-cols-2 gap-4">
                      <div className="text-center p-4 bg-gray-50 dark:bg-gray-800 rounded-lg">
                        <div className="text-2xl font-bold" style={{ color: SDG_COLORS[showSdgModal] }}>
                          {sdgData?.value || 0}
                        </div>
                    ...
                    <div className="flex items-center gap-2 p-3 rounded-lg" style={{ backgroundColor: `${SDG_COLORS[showSdgModal]}20` }}>
                      {isCommitted ? (
                        <>
                          <CheckSquare className="w-4 h-4" style={{ color: SDG_COLORS[showSdgModal] }} />
                          <span className="text-sm" style={{ color: SDG_COLORS[showSdgModal] }}>This is one of your committed SDGs</span>
                        </>
                      ) : (
                        <>
                          <Award className="w-4 h-4" style={{ color: SDG_COLORS[showSdgModal] }} />
                          <span className="text-sm" style={{ color: SDG_COLORS[showSdgModal] }}>Bonus contribution (not in your commitments)</span>
                        </>
                      )}
                    </div>
                    {relatedProjects.length > 0 && (
                      <div>
                        <h3 className="text-gray-800 dark:text-white font-semibold text-sm mb-2">Related Projects:</h3>
                        <div className="space-y-2">
