...
function calculateOrganizationImpactScore(params: {
  ...
  // Updated weights (65% for hours + people as primary drivers): Hours 35%, Ground-Truth Outcomes 30%, Tasks 20%, SDG 10%, Engagement 5%
  const score = Math.round(
    hoursScore * 0.35 +         // Hours contributed (35%)
    beneficiariesScore * 0.30 + // Ground-truth outcomes (30%)
...
export async function getDashboardDataForVolunteer(userId: number, matchThreshold: number = DEFAULT_MATCH_THRESHOLD): Promise<any> {
  ...
  try {
    ...
    const summary = {
      ...
      pendingAssignments,
      completedTasks: volunteerTasks.filter(t => t.status?.toLowerCase() === 'completed').length,
