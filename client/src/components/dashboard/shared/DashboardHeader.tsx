...
// Get menu sections based on user type
function getMenuSections(userType: string | null, navigate: (path: string) => void) {
  if (userType === 'volunteer') {
    return [
      ...
      },
      {
        title: "ACTIONS",
        items: [
          { icon: Plus, label: "Log Impact", desc: "Record hours & outcomes", action: () => navigate('/volunteer/log') },
          { icon: FileText, label: "My History", desc: "View past activities", action: () => navigate('/volunteer/history') },
        ]
      },
      {
        title: "ACCOUNT",
        items: [
          { icon: Settings, label: "Profile Settings", desc: "Manage your profile", action: () => navigate('/volunteer/settings') },
        ]
      },
    ];
  }
    ...
    return [
      {
        ...
        items: [
          ...
          { icon: FolderOpen, label: "Projects", desc: "Manage your projects", action: () => navigate('/ngo/projects') },
          { icon: Shield, label: "Verify", desc: "1-tap verification queue", action: () => navigate('/ngo/verification') },
        ]
      },
      {
        title: "ACTIONS",
      ...
      },
      {
        title: "ACCOUNT",
        items: [
          { icon: Settings, label: "Profile Settings", desc: "Manage organization profile", action: () => navigate('/ngo/settings') },
        ]
      },
    ];
  }
    ...
    return [
      {
        title: "MAIN",
        items: [
          { icon: Home, label: "Dashboard", desc: "ESG overview", action: () => navigate('/dashboard') },
          { icon: FileText, label: "Reports", desc: "Export ESG reports", action: () => navigate('/corporate/reports') },
        ]
      },
      {
        title: "ACCOUNT",
        items: [
          { icon: Settings, label: "Profile Settings", desc: "Manage company profile", action: () => navigate('/corporate/settings') },
        ]
      },
    ];
  }
...
export default function DashboardHeader({
  ...
  const menuSections = getMenuSections(userType, navigate);
  const roleLabel = ROLE_LABELS[userType || ''] || 'Dashboard';
      ...
      >
        <div className="flex items-center justify-between px-5 pb-3">
          ...
          <div className="flex-shrink-0 flex justify-end items-center gap-1" style={{ width: '20%' }}>
            {/* Refresh button for corporate */}
            {userType === 'corporate-partner' && onRefresh && (
              <button
        ...
        <div className="fixed inset-0 z-[100] flex flex-col">
          ...
          <div className="absolute top-0 right-0 w-80 max-w-[90vw] h-full bg-white shadow-2xl flex flex-col animate-in slide-in-from-right duration-200" style={{ paddingTop: 'env(safe-area-inset-top, 0px)', paddingBottom: 'env(safe-area-inset-bottom, 0px)' }}>
            ...
            <div
              ...
              style={{ background: userType === 'volunteer'
                ? 'linear-gradient(to right, #d1fae5 0%, #a7f3d0 100%)'
                : userType === 'organization'
                ? 'linear-gradient(to right, #fef9c3 0%, #dbeafe 50%, #93c5fd 100%)'
                : 'linear-gradient(to right, #e0e7ff 0%, #c7d2fe 100%)'
              }}
            >
              <div>
            ...
            {metrics && (
              <div className="px-4 py-3 bg-slate-50 border-b border-slate-200">
                <div className="flex items-center justify-center gap-4 text-sm">
                  {metrics.activeProjects !== undefined && (
                    <>
                      <div className="flex items-center gap-1.5">
                        <FolderOpen className="w-4 h-4 text-blue-600" />
                        <span className="font-semibold text-slate-800">{metrics.activeProjects}</span>
                        <span className="text-slate-500">Projects</span>
                      </div>
                      <span className="text-slate-300">|</span>
                    </>
                  )}
                  {metrics.activeVolunteers !== undefined && (
                    <div className="flex items-center gap-1.5">
                      <Users className="w-4 h-4 text-emerald-600" />
                      <span className="font-semibold text-slate-800">{metrics.activeVolunteers}</span>
                      <span className="text-slate-500">Volunteers</span>
                    </div>
                  )}
                  {metrics.totalHours !== undefined && (
                    <div className="flex items-center gap-1.5">
                      <Clock className="w-4 h-4 text-amber-600" />
                      <span className="font-semibold text-slate-800">{metrics.totalHours}</span>
