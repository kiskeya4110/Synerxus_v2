...
- **SDG Mapping**: Visual representation and tracking of project alignment with UN SDGs, with multi-tenant security. Features interactive **spider web (radar) chart** comparing organization's selected SDG focus areas from Settings (blue baseline) vs actual project distribution (orange-gold normalized data), showing alignment between stated priorities and execution. **Project Statistics Dashboard** (Nov 2025) - displays four summary cards above radar chart showing Total Projects, Completed Projects, Active Projects, and Average Completion percentage aggregated from organization's projects. **Enhanced Project Cards** - Connected Projects section now displays completion percentage with progress bar, location info, and related SDG badges showing cross-SDG alignment. **Interactive Connected Projects** - clicking any SDG goal displays related projects with clickable cards that navigate to project detail pages.
